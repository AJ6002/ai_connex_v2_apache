...
export const CompilerView: React.FC<CompilerViewProps> = ({
  ...
  const triggerCompilation = async (file: File) => {
    setIsCompiling(true);
    ...
    const apiCall = fetch('http://localhost:8000/api/v1/compile', {
      ...
      if (data && data.first_csv) {
        try {
          ...
          const targetColumn = lowerName.includes('insurance') ? 'charges' : (lowerName.includes('house_prices') ? 'SalePrice' : (lowerName.includes('manufacturing') ? 'RUL' : ''));
          if (targetColumn) {
    ...
    <div className="space-y-6 text-primary">
      ...
      {showDagModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xl animate-fadeIn">
          <div className="glass-panel w-full max-w-2xl p-6 sm:p-8 rounded-3xl border-2 border-tas-red/40 shadow-2xl space-y-6 relative overflow-hidden"
