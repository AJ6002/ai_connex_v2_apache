...
# Install React/Vite Frontend dependencies (Required for Node UI)
cd aic/frontend
npm install
cd ../..
```

> 💡 **Important Note for Developers**: Node dependencies (`node_modules/`) are excluded from Git repository tracking to prevent repo bloat. Always run `npm install` inside `aic/frontend/` after cloning or pulling new commits.
