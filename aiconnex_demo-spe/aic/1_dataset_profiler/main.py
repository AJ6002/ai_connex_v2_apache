...
# ── GAP 2: Master Config Endpoints ──────────────────────────────────────────
MASTER_CONFIG_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "master_config.json")
...
def get_master_config():
    ...
    return {
        "status": "success",
        "system_name": "AI-Connexx Suite",
        "active_environment": "local_dev",
        "max_parallel_jobs": 4,
        "default_partition_ratio": "70/15/15"
    }

@app.post("/api/v1/master/config")
def save_master_config(payload: MasterConfigPayload):
    new_cfg = payload.config or {}
    try:
        with open(MASTER_CONFIG_FILE, "w", encoding="utf-8") as f:
            json.dump(new_cfg, f, indent=2)
        return {"status": "success", "message": "Master config saved", "config": new_cfg}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to save master config: {str(e)}")

# ── GAP 3: Workspace Files List Endpoint ──────────────────────────────────
...
def get_workspace_files():
    ...
    if os.path.exists(ws_dir):
        for root, dirs, files in os.walk(ws_dir):
            for f in files:
                if f.endswith((".csv", ".zip", ".json", ".pkl")):
...
async def compile_dataset(
    ...
    safe_stem = "ds_" + hashlib.md5(raw_stem.encode()).hexdigest()[:10] if len(raw_stem) > 30 else raw_stem
    
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    workspace_data_dir = os.path.join(base_dir, "workspace_data", safe_stem)
    os.makedirs(workspace_data_dir, exist_ok=True)
    
    saved_file_path = os.path.join(workspace_data_dir, file.filename)
    with open(saved_file_path, "wb") as fout:
        fout.write(contents)
        
    relative_path = os.path.relpath(saved_file_path, base_dir).replace("\\", "/")
    ...
    for col in df.columns[:15]:
        ...
        badge = "bg-blue-100 text-blue-800" if "int" in dtype or "float" in dtype else "bg-emerald-100 text-emerald-800"
        preview_cols.append({"name": str(col), "type": dtype, "badge": badge})
        
    preview_rows = df.head(5).to_dict(orient="records") if len(df) > 0 else []
    
    ts_col = next((c for c in df.columns if any(k in c.lower() for k in ['date', 'time', 'timestamp'])), "time_cycle")
    grp_col = next((c for c in df.columns if any(k in c.lower() for k in ['unit', 'id', 'device', 'station', 'group'])), "unit_id")

    # Sanitize preview rows: replace NaN/inf with None for JSON compliance
    import math
    def safe_val(v):
        if v is None:
            return None
    ...
    raw_preview = df.head(5).to_dict(orient="records") if len(df) > 0 else []
    preview_rows = [{k: safe_val(v) for k, v in row.items()} for row in raw_preview]


    return {
        "status": "success",
        "first_csv": relative_path,
        "combined_file": relative_path,
        "merged_files": [file.filename],
        "total_rows": num_rows,
        "total_cols": num_cols,
        "schema_map": {
            "canonical_timestamp_col": ts_col,
            "canonical_group_col": grp_col
        },
        "preview_columns": preview_cols,
        "preview_rows": preview_rows,
        "join_audits": [{"cartesian_guard_passed": True}],
        "dataset_card": {
            "domain_detected": "Industrial Sensor Telemetry",
            "rul_synthesis_applied": False,
            "output_paths": {}
        }
    }
...
async def profile_dataset(
    ...
    if file is not None:
        ...
        # Resolve file_path against base_dir (aic root) if relative
        abs_file_path = file_path if os.path.isabs(file_path) else os.path.normpath(os.path.join(base_dir, file_path))
        if not os.path.exists(abs_file_path):
            raise HTTPException(status_code=404, detail=f"File not found at specified file_path: {file_path}")
        orig_filename = os.path.basename(abs_file_path)
        filename = orig_filename.lower()
        with open(abs_file_path, "rb") as f:
            contents = f.read()
    else:
        raise HTTPException(status_code=400, detail="Either file upload or file_path parameter is required.")

    if not (filename.endswith('.csv') or filename.endswith('.json') or filename.endswith('.txt') or filename.endswith('.xlsx') or filename.endswith('.xls')):
        raise HTTPException(
            status_code=400,
            detail="Unsupported file format. Please upload a CSV (.csv), JSON (.json), XLSX (.xlsx), or TXT (.txt) file."
        )
    ...
    try:
        ...
        os.makedirs(workspace_data_dir, exist_ok=True)
        saved_file_path = os.path.join(workspace_data_dir, orig_filename[:80])  # truncate filename too
        try:
            with open(saved_file_path, "wb") as fout:
                fout.write(contents)
        except OSError:
