...
def feature_engineer_data(payload: FeatureEngineerPayload):
    try:
        ...
        if not os.path.isabs(prep_path):
            candidate_path = os.path.normpath(os.path.join(base_dir, prep_path))
            if os.path.exists(candidate_path):
                prep_path = candidate_path

        if not os.path.exists(prep_path):
            raise HTTPException(status_code=404, detail=f"Prepared dataset file not found at {payload.prepared_file_path}")
        ...
        engineered_path = os.path.join(workspace_data_dir, f"engineered_{run_id}.csv")
