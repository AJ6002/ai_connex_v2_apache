...
def _run_training_job(job_id: str, payload: TrainPayload):
    ...
    try:
        # ── Path resolution against AIC_ROOT & fallback resolver ─────────
        base_dir = AIC_ROOT
        ...
        manifest_path = resolve_file_path(manifest_path)
        ...
        anomaly_algos = ["isolationforest", "oneclasssvm", "kmeans", "localoutlierfactor"]
        if any(a in algo_lower for a in anomaly_algos) or "anomaly" in family_type.lower() or "anomaly" in task_type.lower() or "anomaly" in ml_task.lower():
            is_anomaly = True
            is_regression = False
        elif y_train is not None and (pd.api.types.is_float_dtype(y_train) or y_train.nunique() > 20):
            is_anomaly = False
            is_regression = True
        ...
        if is_anomaly:
            ...
            # Fallback for supervised model missing y: fit on dummy target or last column
            y_fallback = df_train.iloc[:, -1]
            model.fit(X_train_scaled, y_fallback)
