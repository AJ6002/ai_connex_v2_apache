...
import { Send, MessageSquare, Sparkles, Cpu, Layers, Zap, Activity, Check, RefreshCw, Bot, User } from 'lucide-react';
...
export const MainChatView: React.FC<MainChatViewProps> = ({ darkMode, onSelectStarter }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-7rem)] py-8 px-4 pl-16 sm:pl-20 max-w-5xl mx-auto">
      ...
      >
        {/* Messages Container */}
        <div className="space-y-4 max-h-[380px] overflow-y-auto pr-2 mb-6 custom-scrollbar">
          {messages.map((msg) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`p-4 rounded-2xl max-w-[92%] sm:max-w-[85%] text-xs sm:text-sm font-mono leading-relaxed shadow-sm border ${
                    isUser
          ...
          {isGenerating && (
            <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-100 dark:bg-slate-800/80 w-fit text-xs font-mono text-slate-600 dark:text-slate-300 animate-pulse border border-slate-200 dark:border-slate-700">
              ...
              <span>AI Connexx compiler matching DAG schemas & training recipes...</span>
            </div>
          )}
