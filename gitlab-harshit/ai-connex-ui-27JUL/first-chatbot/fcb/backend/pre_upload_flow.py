...
from pre_upload_extraction import extract_pre_upload_turn, GEMINI_MODEL
...
def _check_escalation(
    ...
    """Check if we should offer quick-select instead of another free-text question.
    Returns True if escalation is needed."""
    # Check 1: ambiguity detected for 2+ consecutive turns
    ...
    # Check 2: same field asked 2+ times without being filled
    if len(replies) >= 2:
        # Count how many times the last question was asked
...
def process_turn(
    ...
    if is_complete:
        ...
        reply = (
            "Great, I have all the information I need! You can now upload your dataset "
        ...
        reply = (
            ...
            + (question or "Could you try rephrasing that?")
        )
    elif question:
        contract.planning_hints.recommended_next_action = "ask_clarification"
        reply = question
    else:
        # All required + recommended fields are filled, but we might still
        # have opportunistic fields to fill. Just acknowledge and prompt.
        contract.planning_hints.recommended_next_action = "prompt_for_upload"
        reply = (
            "Thanks! I have a good understanding of your needs. "
            "Feel free to upload your dataset whenever you're ready."
        )

    # Update metadata
    contract.metadata.processing_time_ms = int((time.time() - start_time) * 1000)
    contract.metadata.llm_model = GEMINI_MODEL
