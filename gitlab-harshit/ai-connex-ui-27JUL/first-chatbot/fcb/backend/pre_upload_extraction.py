...
def _build_system_prompt(contract: PreUploadContract) -> str:
    ...
    return (
        "You are the pre-upload intent-gathering layer for the AI Connexx assistant.\n\n"
...
def _call_gemini(
    ...
    """Call Gemini with structured-output mode and return raw JSON text."""
    ...
    response = client.models.generate_content(
        ...
        contents=contents,
        config={
            "system_instruction": _build_system_prompt(contract),
            "temperature": 0.1,
...
def _fallback_extract(message: str, contract: PreUploadContract) -> dict:
    ...
    file_type_patterns = [
        ...
        (r"\.parquet\b", "Parquet"), (r"\.avro\b", "Avro"),
...
def extract_pre_upload_turn(
    ...
    if os.environ.get("GEMINI_API_KEY"):
        try:
            raw_text = _call_gemini(message, history, contract)
