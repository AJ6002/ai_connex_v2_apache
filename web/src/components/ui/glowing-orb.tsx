...
export const GlowingOrb: FC<GlowingOrbProps> = ({
  className,
  hue = 0,
  rotationSpeed = 0.4,
  hoverIntensity = 0.2,
    ...
    try {
      ...
      return () => {
        ...
        if (container && glContext && glContext.canvas) {
          try {
            const canvasNode = glContext.canvas as Node;
            if (container.contains(canvasNode)) {
      ...
      console.error("Error initializing Glowing Orb:", error);
  ...
  }, [hue, rotationSpeed, hoverIntensity, vert, frag]);
