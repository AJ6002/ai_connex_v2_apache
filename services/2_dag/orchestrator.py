...
class PipelineRun:
    ...
    def _load_recipe(self) -> dict:
        ...
        # Fallback: query central transit mapping JSON database directly for any arbitrary DAG ID
        db_path = os.path.join(root_dir, "Documentation", "transit_mappings", "algorithm_dags_transit_mapping.json")
        if os.path.exists(db_path):
            try:
                with open(db_path, "r", encoding="utf-8") as f:
                    db = json.load(f)
                resolved_dag_id = self.dag_id
                if resolved_dag_id not in db:
                    # Map family to fallback ID
                    fallback_ids = {
                    ...
                    resolved_dag_id = fallback_ids.get(self.algorithm_family, "DAG_001")
                
                if resolved_dag_id in db:
                    recipes = db[resolved_dag_id]["recipes"]
                    return {
                        "prepare_recipe": recipes["prepare_recipe"],
                        "feature_engineering_recipe": recipes["feature_engineering_recipe"],
                        "splitting_recipe": recipes["splitting_recipe"],
                        "training_recipe": recipes["training_recipe"]
                    }
            except Exception as db_err:
                print("Error loading from transit mapping database in orchestrator:", db_err)
                
        # File Fallback: load separate recipe files directly from 3_recipe_orchestrator/recipe
    ...
    def _sync_manifest(self):
        try:
            if hasattr(self, "manifest_path") and os.path.exists(self.manifest_path):
                manifest_dir = os.path.dirname(self.manifest_path)
                target_manifest = os.path.join(manifest_dir, "manifest.json")
                import shutil
                shutil.copy2(self.manifest_path, target_manifest)
                ...
                sqlite_tracker.update_run_manifest(self.run_id, manifest_data)
        except Exception as e:
    ...
    async def execute(self):
        ...
        dataset_name = self.profile.get("filename") or os.path.basename(raw_file_path)
        dataset_stem = os.path.splitext(dataset_name)[0]
        if dataset_stem.startswith("profiled_"):
            dataset_stem = dataset_stem[9:]
        elif dataset_stem.startswith("compiled_"):
            dataset_stem = dataset_stem[9:]

        # Create dag_<dataset_stem>.csv inside workspace_dir by copying from raw_file_path
        dag_file_path = os.path.normpath(os.path.join(workspace_dir, f"dag_{dataset_stem}.csv"))
        if os.path.exists(raw_file_path):
            try:
                import shutil
                shutil.copy2(raw_file_path, dag_file_path)
                self.add_log("INFO", f"Copied dataset to: {dag_file_path}")
            except Exception as e:
                self.add_log("WARNING", f"Could not copy to dag_file_path: {e}")
                dag_file_path = raw_file_path
        else:
            dag_file_path = raw_file_path
        ...
        # Log DAG step to SQLite tracker
        try:
            import sys
            sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            import sqlite_tracker
            sqlite_tracker.init_run(
                run_id=self.run_id,
            ...
            sqlite_tracker.log_step(
                run_id=self.run_id,
                step_name="DAG Assignment (Node 2)",
                status="success",
                output_file=self.dag_file_path,
                metrics={"dag_id": self.dag_id}
            )
        except Exception as sqle:
            print(f"Error logging DAG step to SQLite: {sqle}")
        ...
        try:
            for idx, step in enumerate(self.steps):
                ...
                # Sync manifest and log step to SQLite tracker
                self._sync_manifest()
                try:
                    step_output_file = ""
                    step_metrics = {}
                    if step_name.startswith("Data Preparation"):
                        step_output_file = self.prepared_file_path or ""
                    elif step_name.startswith("Feature Engineering"):
                        step_output_file = self.engineered_file_path or ""
                    elif step_name.startswith("Data Splitting"):
                        step_output_file = self.train_path or ""
                        step_metrics = {"train_path": self.train_path, "val_path": self.val_path, "test_path": self.test_path}
                    elif step_name.startswith("Model Training"):
                        step_output_file = self.model_path or ""
                        step_metrics = {"model_path": self.model_path, "scaler_path": self.scaler_path}
                    elif step_name.startswith("Evaluation"):
                        step_output_file = self.manifest_path
                        step_metrics = self.eval_metrics
                    elif step_name.startswith("Deployment"):
                        step_output_file = self.deploy_result.get("model_file") or ""
                        step_metrics = {"endpoint_url": self.deploy_result.get("endpoint_url")}
                    ...
                    print(f"Error logging step to SQLite: {sqle}")
            ...
            try:
                import sqlite_tracker
                sqlite_tracker.update_run_status(self.run_id, "completed")
            except Exception:
                pass
            ...
            try:
                import sqlite_tracker
                sqlite_tracker.update_run_status(self.run_id, "failed")
            except Exception:
                pass
    ...
    async def _run_step_api(self, step_name: str):
        ...
        if step_name == "Data Preparation (PREPARE)":
            ...
            response = await loop.run_in_executor(None, lambda: requests.post(
                prep_url,
                json={
                    "raw_file_path": self.dag_file_path,
                    "recipe": prep_recipe,
                    "run_id": self.run_id,
...
class FIFOQueue:
    ...
    def enqueue(self, run: PipelineRun):
        node = QueueNode(run)
        ...
        self.size += 1
        run.status = "queued"
        run.add_log("INFO", f"Pipeline run enqueued in backend FIFO Queue. Position in queue: {self.size}")

    def dequeue(self) -> Optional[PipelineRun]:
...
async def queue_processor_loop():
    ...
    while True:
        async with QUEUE_LOCK:
            run = PIPELINE_QUEUE.dequeue()
        if run is not None:
            run.status = "running"
            try:
                await run.execute()
            except Exception as e:
                # Internally run.execute logs its own exceptions
                pass
        else:
...
def create_pipeline_run(profile: dict) -> PipelineRun:
    ...
    # Enqueue in the FIFO queue
    PIPELINE_QUEUE.enqueue(run)
    
    # Start the queue processor loop if it hasn't been started
