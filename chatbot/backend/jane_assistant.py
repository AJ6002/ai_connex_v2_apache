...
def get_kb_context(user_input: str, tenant_id: str = "global", session_id: str = "") -> str:
    ...
    try:
        ...
        if prompt_ctx and prompt_ctx.strip():
            ...
            return prompt_ctx[:3000]
        return "No specific Knowledge Base records matched this query."
    except Exception as exc:
        logger.warning(f"[JaneKB] Retrieval query failed: {exc}")
        return (
            "[⚠️ System Notice: Knowledge Base query encounter error: " + str(exc) +
            ". Operating in fallback reasoning mode.]"
        )

# ==============================================================================
# 4. MAIN JANE ASSISTANT EXECUTION PIPELINE
# ==============================================================================
def run_jane_assistant(
    ...
    # 2. Get Grounded KB Context
    rag_context = retrieved_rag_docs if retrieved_rag_docs is not None else get_kb_context(user_input, tenant_id=tenant_id, session_id=session_id)
...
{user_input}"""
    ...
    # Render high-fidelity Mistune HTML
    try:
...
if __name__ == "__main__":
    print("Testing Jane Assistant Engine with 6-Layer KB & Qwen LLM...")
