...
"""
    ...
    def _resolve_manifest_path(self, run_id: str) -> Optional[str]:
        ...
        candidates = [
            # From orchestrator's PipelineRun.manifest_path
    ...
    def _find_workspace_file(self, name: str) -> Optional[str]:
        ...
        wd = BASE_DIR / "workspace_data"
        if not wd.exists():
            return None
        # Direct hit at flat level
        if (wd / name).exists():
            return str(wd / name)
        # Recursive search exact match
        for p in wd.rglob(name):
            if p.is_file():
                return str(p)
        # Recursive search pattern match (e.g. train_*run_id*.csv)
        parts = name.split("_run_")
        if len(parts) == 2:
            prefix, run_part = parts[0], parts[1].replace(".csv", "").replace(".pkl", "")
            for p in wd.rglob(f"{prefix}*{run_part}*"):
                if p.is_file():
                    return str(p)
        return None
