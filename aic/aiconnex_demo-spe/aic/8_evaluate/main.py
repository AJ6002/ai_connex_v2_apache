...
def evaluate_model(payload: EvaluatePayload):
    try:
        ...
        # ── Path resolution against AIC_ROOT ─────────────────────────────
        base_dir = AIC_ROOT
        ...
        model_path = resolve_file_path(model_path)
        test_path  = resolve_file_path(test_path)
        train_path = resolve_file_path(train_path)
        val_path   = resolve_file_path(val_path)

        # ── Retry loop for model file creation (wait up to 5s if background train is finishing) ─────
        import time
        for _ in range(10):
        ...
        if not model_path or not os.path.exists(model_path):
            raise HTTPException(status_code=404, detail=f"Model file not found at {payload.model_path}")
        if not test_path or not os.path.exists(test_path):
            raise HTTPException(status_code=404, detail=f"Test dataset not found at {payload.test_path}")
        ...
        if not target_col:
            target_col = manifest.get("target_column") or manifest.get("schema_config", {}).get("target_column")

        print(f"[Evaluator] ml_task='{ml_task}' | topology='{topology}' | entity='{entity_col}' | target='{target_col}'")
        ...
        if not scaler_path and model_path:
            cand_scaler = model_path.replace("model_", "scaler_")
            cand_scaler = resolve_file_path(cand_scaler)
            if cand_scaler and os.path.exists(cand_scaler):
            ...
            if scaler is not None and hasattr(scaler, "feature_names_in_"):
                ...
                if X.shape[1] != expected_n:
                    if X.shape[1] == expected_n + 1:
                        # Drop last column (likely target)
                        X = X.iloc[:, :expected_n]
