...
SERVICES = [
    {"name": "Dataset Profiler",        "path": "services/1_dataset_profiler/main.py",     "port": 8000},
    {"name": "DAG Orchestrator",        "path": "services/2_dag/main.py",                  "port": 8001},
    {"name": "Recipe Orchestrator",     "path": "services/3_recipe_orchestrator/main.py",  "port": 8002},
    {"name": "Prepare API",             "path": "services/4_prepare/main.py",              "port": 8003},
    {"name": "Feature Engineering API", "path": "services/5_feature_engineering/main.py", "port": 8004},
    {"name": "Split API",               "path": "services/6_split/main.py",                "port": 8005},
    {"name": "Train API",               "path": "services/7_train/main.py",                "port": 8006},
    {"name": "Evaluate API",            "path": "services/8_evaluate/main.py",             "port": 8007},
    {"name": "Deploy API",              "path": "services/9_deploy_monitor/main.py",       "port": 8008},
]
...
def launch_service(svc):
    ...
    if not os.path.exists(full_path) and svc["path"].startswith("services/"):
        alt_path = svc["path"].replace("services/", "", 1)
        if os.path.exists(os.path.join(BASE_DIR, alt_path)):
            full_path = os.path.join(BASE_DIR, alt_path)
