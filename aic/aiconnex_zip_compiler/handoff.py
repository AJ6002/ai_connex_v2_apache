...
def export_compiler_handoff(
    ...
    dataset_name = Path(zip_filename).stem
    if len(all_dfs) > 1:
        combined_csv_path = output_dir / f"compiled_{dataset_name}.csv"
        ...
        combined_csv_path = output_dir / f"compiled_{dataset_name}.csv"
        first_df = list(merged_dfs.values())[0]
        first_df.to_csv(combined_csv_path, index=False)
